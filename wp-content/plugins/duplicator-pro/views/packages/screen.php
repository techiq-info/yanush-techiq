...
add_filter('set_screen_option_package_screen_options', array('DUP_PRO_Package_Screen', 'packages_set_option'), 11, 3);
...
class DUP_PRO_Package_Screen extends DUP_PRO_UI_Screen
{
    ...
    public function Init()
    {
        ...
        switch (strtoupper($active_tab)) {
            case 'LIST': $content = $this->get_list_help();
                break;
            case 'NEW1': $content = $this->get_step1_help();
                break;
            case 'NEW2': $content = $this->get_step2_help();
                break;
            case 'DETAIL': $content = $this->get_details_help();
                break;
            default:
    ...
    public function get_list_help()
    {
        ...
        $result .= '<b><i class="fa fa-archive"></i> '.DUP_PRO_U::__("Packages » All").'</b>';
        $result .= '<br/>';
        ...
        $result .= '<b><i class="fa fa-download"></i> '.DUP_PRO_U::__("Downloads").'</b>';
        $result .= '<br/>';
        ...
        $result .= '<b><i class="fa fa-bars"></i> '.DUP_PRO_U::__("More Items").'</b>';
        $result .= '<br/>';
        $result .= DUP_PRO_U::__("To see the details, transfer or view remote store locations of a package click the 'More Items' menu button. If a package contains remote storage endpoints a
					blue dot will show as &nbsp; <i class='fa fa-bars remote-data-pass'></i> &nbsp; on the more items menu button.  If a red icon shows 
					<i class='fa fa-bars remote-data-fail'></i> &nbsp; then one or more of the storage locations failed during the transfer phase.");
        $result .= '<br/><br/>';

        $result .= '<b><i class="far fa-file-archive fa-sm"></i> '.DUP_PRO_U::__("Archive Types").'</b>';
        $result .= '<br/>';
        ...
        $result .= '<b><i class="fa fa-bolt"></i> '.DUP_PRO_U::__("How to Install a Package").'</b>';
        $result .= '<br/>';
        ...
        $result .= '<b><i class="fas fa-undo-alt"></i> '.DUP_PRO_U::__("The Recovery Point").'</b>';
        $result .= '<br/>';
    ...
    public static function packages_set_option($status, $option, $value)
    {
        $user_id = get_current_user_id();
