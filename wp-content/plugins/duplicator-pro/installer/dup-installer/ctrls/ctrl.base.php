...
class DUPX_CTRL
{
        ...
        switch ($ctrlAction) {
            case "ctrl-step1" :
                if ($stepAction === DUPX_CTRL::ACTION_STEP_SET_TEMPLATE) {
                    $paramsManager->setValueFromInput(DUPX_Paramas_Manager::PARAM_TEMPLATE);
                    DUPX_Log::info('NEW TEMPLATE:'.$paramsManager->getValue(DUPX_Paramas_Manager::PARAM_TEMPLATE));
                    $paramsManager->save();
                    DUPX_Template::getInstance()->setTemplate($paramsManager->getValue(DUPX_Paramas_Manager::PARAM_TEMPLATE));
                }
                $this->pageView = 'step1';
                break;
            case "ctrl-step2" :
    ...
    public function renderPage()
    {
        ...
        $echo                                 = false;
        $paramsManager                        = DUPX_Paramas_Manager::getInstance();
        $this->extraParamsPage['bodyClasses'] = 'template_'.$paramsManager->getValue(DUPX_Paramas_Manager::PARAM_TEMPLATE);
