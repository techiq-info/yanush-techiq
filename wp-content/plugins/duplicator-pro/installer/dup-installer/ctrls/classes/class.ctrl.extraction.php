...
class DUP_PRO_Extraction
{
    ...
    protected function runChunkExtraction()
    {
        ...
        if ($zip->open($this->archive_path) == true) {
            ...
            do {
                ...
                do {
                    ...
                    if (!$skip && $this->archive_action === self::ACTION_SKIP_CORE_FILES && DupProSnapLibUtilWp::isWpCore($filename, DupProSnapLibUtilWp::PATH_RELATIVE)) {
                        $skip = true;
                    }
    ...
    protected function runZipArchive()
    {
    ...
    public function setFilePermission()
    {
        ...
        if ($this->set_file_perms || $this->set_dir_perms || (($this->archive_engine == self::ENGINE_ZIP_SHELL) && ($this->zip_filetime == 'current'))) {
            ...
            foreach ($objects as $name => $object) {
                ...
                if ($set_file_mtime && is_writable($name)) {
                    @touch($name);
                }
            }
        }
