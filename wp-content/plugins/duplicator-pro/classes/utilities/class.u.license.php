    ...
    public static function getLicenseStatus($forceRefresh)
    {
        ...
        if (self::isValidOvrKey($license_key)) {
            ...
            if ($global->license_status == DUP_PRO_License_Status::Uncached) {
                ...
                $global->license_expiration_time = time() + DUPLICATOR_PRO_LICENSE_CACHE_TIME;

                $global->save();

                DUP_PRO_LOG::trace("Set cached value from with expiration ".DUPLICATOR_PRO_LICENSE_CACHE_TIME." seconds from now ({$global->license_expiration_time})");
            }
        }
