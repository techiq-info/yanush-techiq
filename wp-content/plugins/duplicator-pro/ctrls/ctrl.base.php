...
class DUP_PRO_CTRL_Base
{
    ...
    public static function getCurrentPageTabId()
    {
        $page = self::getCurrentPageId();
        if (strlen($page) == 0) {
            return '';
        }
        $tab = DupProSnapLibUtil::filterInputRequest('tab', FILTER_SANITIZE_STRING, array('options' => array('default' => '')));
        return $page.(strlen($tab) ? '_'.$tab : '');
    }
